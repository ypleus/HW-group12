...
class AES:
    ...
    MIX_C = [[0x2, 0x3, 0x1, 0x1], [0x1, 0x2, 0x3, 0x1], [0x1, 0x1, 0x2, 0x3],
             [0x3, 0x1, 0x1, 0x2]]
    I_MIXC = [[0xe, 0xb, 0xd, 0x9], [0x9, 0xe, 0xb, 0xd], [0xd, 0x9, 0xe, 0xb],
              [0xb, 0xd, 0x9, 0xe]]
    RCon = [
    ...
    def SubBytes(self, State):
        return [
            self.S_BOX[i][j] for i, j in [(_ >> 4, _ & 0xF) for _ in State]
        ]

    def SubBytes_Inv(self, State):
        return [
            self.I_SBOX[i][j] for i, j in [(_ >> 4, _ & 0xF) for _ in State]
        ]
    ...
    def ShiftRows_Inv(self, S):
        return [
            S[0], S[13], S[10], S[7], S[4], S[1], S[14], S[11], S[8], S[5],
            S[2], S[15], S[12], S[9], S[6], S[3]
        ]
        ...
        for row in range(4):
            for col in range(4):
                for Round in range(4):
                    M[row + col * 4] ^= self.mul(M1[row][Round],
                                                 M2[Round + col * 4])
    ...
    def round_key_generator(self, _16bytes_key):
        w = [
            _16bytes_key >> 96, _16bytes_key >> 64 & 0xFFFFFFFF,
            _16bytes_key >> 32 & 0xFFFFFFFF, _16bytes_key & 0xFFFFFFFF
        ] + [0] * 40
        for i in range(4, 44):
            temp = w[i - 1]
            if not i % 4:
        ...
        return [
            self.num_2_16bytes(
                sum([
                    w[4 * i] << 96, w[4 * i + 1] << 64, w[4 * i + 2] << 32,
                    w[4 * i + 3]
                ])) for i in range(11)
        ]
...
def encrypt(plaintext, RK):
    plaintext_bytes = aes.num_2_16bytes(plaintext)
    ciphertext = aes.aes_encrypt(plaintext_bytes, RK)
    return aes._16bytes2num(ciphertext)


def main():

    key = 0x00000000000000000000000000000001
    RK = aes.round_key_generator(key)
    num_threads = 16
    num_strings = 1000
